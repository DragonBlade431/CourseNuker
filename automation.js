...
async function runAutomation(
  ...
  // Queue ALL lectures — don't trust VTU's is_completed flag from the
  // course listing; it can be true after a single view even when progress
  // hasn't reached 100%.  tryOnce() will verify actual completion.
  let pending = [];
  for (let i = 0; i < lectures.length; i++) {
    pending.push({ lec: lectures[i], idx: i + 1 });
  }
