...
async function login() {
    if (!EMAIL || !PASSWORD) {
        ...
        console.error("CourseNuker CLI requires VTU_EMAIL and VTU_PASSWORD in .env\n");
...
async function requestWithAuth(config, retry = true) {
    try {
        ...
        if (retry && (status === 401 || status === 419 || status === 403)) {
            console.log("⚠ Session expired. Re-infiltrating...");
            isLoggedIn = false;
            await login();
    ...
    try {
        ...
        while (attempt < maxAttempts) {
            ...
            if (percent === 100 && is_completed === true) {
                console.log(`  ✓ Success! Lecture nuked.`);
                return true;
            }
        }
...
async function run() {
    ...
    if (!EMAIL || !PASSWORD) {
        console.error("\n" + "=".repeat(60));
        console.error("✗ CourseNuker CLI: Missing Credentials");
        console.error("=".repeat(60));
    ...
    console.log("========================================");
    console.log("   ⚔️  CourseNuker — Blood for the GPA God");
    console.log("========================================");
    ...
    console.log("\n========================================");
    console.log(`   Nuking Lectures (Parallel: ${BATCH_SIZE}/batch)...`);
    console.log("========================================");
    ...
    // Summary
    console.log("\n========================================");
    console.log("   ⚔️  GG EZ — Not even close, baby.");
    console.log("========================================");
