...
// Keys
const STUDENTS_KEY = "coursenuker:students";
const LECTURES_KEY = "coursenuker:lectures";
...
const QUEUE_KEY    = "coursenuker:queue";     // Redis List  — jobIds only (no creds)
const JOB_PREFIX   = "coursenuker:job:";      // Hash per job
const DEDUP_PREFIX = "coursenuker:dedup:";    // String  dedupKey → jobId
...
async function enqueueJobId(jobId) {
  ...
  try {
    ...
    // Keep the queue list itself alive; the TTL is a rolling maximum wait time
    await redis.expire(QUEUE_KEY, JOB_TTL_SEC * 4); // 4 hours generous upper bound
  } catch (e) {
...
async function setDedupKey(dedupKey, jobId) {
  if (!redis) return;
  // No TTL — the key is explicitly deleted when the job finishes or fails.
  // Crash recovery is handled by restoreQueueFromRedis which calls deleteDedupKey
  // for any stale queued jobs found on startup.
  try { await redis.set(`${DEDUP_PREFIX}${dedupKey}`, jobId); } catch (_) {}
}
